  ...
  "src/utils/storage/ReactNativeAsyncStorage.ts"() {
    ...
    ReactNativeAsyncStorage = class {
      constructor() {
        this.prefix = "aegis:";
        try {
          this.AsyncStorage = (init_module(), __toCommonJS(module_exports)).default;
        } catch (error) {
          throw new AegisError(
            "STORAGE_ERROR" /* STORAGE_ERROR */,
            "STORAGE_ACCESS_DENIED" /* STORAGE_ACCESS_DENIED */,
...
__export(web_exports, {
  ACCOUNT_CLASS_HASHES: () => ACCOUNT_CLASS_HASHES,
  AegisError: () => AegisError,
  AegisErrorCode: () => AegisErrorCode,
  AegisErrorFactory: () => AegisErrorFactory,
  AegisErrorType: () => AegisErrorType,
  AegisSDK: () => AegisSDK,
  ...
  WebAsyncStorage: () => WebAsyncStorage,
  WebSecureStorage: () => WebSecureStorage,
  deriveAddressFromPublicKey: () => deriveAddressFromPublicKey,
  estimateDeploymentFee: () => estimateDeploymentFee,
  expandConfigWithDefaults: () => expandConfigWithDefaults,
  generateAccountAddress: () => generateAccountAddress,
  generateMultipleAccountAddresses: () => generateMultipleAccountAddresses,
  getAccountClassHash: () => getAccountClassHash,
  getConstructorCalldata: () => getConstructorCalldata,
  getPublicKeyFromPrivateKey: () => getPublicKeyFromPrivateKey,
  getSupportedAccountTypes: () => getSupportedAccountTypes,
  isAccountTypeSupported: () => isAccountTypeSupported,
  isValidAccountAddress: () => isValidAccountAddress,
  normalizePrivateKey: () => normalizePrivateKey,
  prepareDeploymentData: () => prepareDeploymentData,
  validateAccountClassHash: () => validateAccountClassHash,
  validateAegisConfig: () => validateAegisConfig,
  validateCallData: () => validateCallData,
  validateDeploymentData: () => validateDeploymentData,
  validateInternalConfig: () => validateInternalConfig,
  validatePrivateKey: () => validatePrivateKey,
  validateStarknetAddress: () => validateStarknetAddress
});
...
var CHAIN_IDS = {
  mainnet: import_starknet.constants.StarknetChainId.SN_MAIN,
  sepolia: import_starknet.constants.StarknetChainId.SN_SEPOLIA,
  devnet: void 0
...
var NetworkManager = class {
  ...
  createProvider(network) {
    ...
    try {
      ...
      return new import_starknet.RpcProvider(providerOptions);
    } catch (error) {
      throw new AegisError(
        "NETWORK_ERROR" /* NETWORK_ERROR */,
        "PROVIDER_CONNECTION_FAILED" /* PROVIDER_CONNECTION_FAILED */,
...
function generateAccountAddress(privateKey, accountType = "argentx") {
  try {
    ...
    const starkKeyPub = import_starknet2.ec.starkCurve.getStarkKey(normalizedKey);
    ...
    const contractAddress = import_starknet2.hash.calculateContractAddressFromHash(
      starkKeyPub,
      classHash,
      constructorCalldata,
      0
...
function getConstructorCalldata(privateKey, accountType = "argentx") {
  ...
  switch (accountType) {
    case "argentx":
      return import_starknet2.CallData.compile({
        owner: starkKeyPub,
        guardian: "0x0"
...
function deriveAddressFromPublicKey(publicKey, accountType = "argentx") {
  try {
    ...
    switch (accountType) {
      case "argentx":
        constructorCalldata = import_starknet2.CallData.compile({
          owner: publicKey,
          guardian: "0x0"
        });
        break;
...
function getPublicKeyFromPrivateKey(privateKey) {
  try {
    const normalizedKey = normalizePrivateKey(privateKey);
    return import_starknet2.ec.starkCurve.getStarkKey(normalizedKey);
  } catch (error) {
    throw new AegisError(
      "WALLET_ERROR" /* WALLET_ERROR */,
      "INVALID_PRIVATE_KEY" /* INVALID_PRIVATE_KEY */,
...
var InAppWallet = class _InAppWallet {
  constructor(privateKey, network, provider, config, storageManager, keyManager, accountType = "argentx") {
    ...
    this.account = new import_starknet3.Account(provider, this.address, privateKey);
  }
  /**
  ...
  async getETHBalance() {
    try {
      ...
      const contract = new import_starknet3.Contract(ERC20_ABI, ETH_ADDRESS, this.provider);
  ...
  async getERC20Balance(tokenAddress, decimals) {
    try {
      const contract = new import_starknet3.Contract(ERC20_ABI, tokenAddress, this.provider);
      const balance = await contract.balanceOf(this.address);
      let balanceStr = balance.toString();
